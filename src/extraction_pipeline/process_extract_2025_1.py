...
#
# THe quick approach is to modify the prompt here, and also to use a different directory so as to not overwrite the previous work
# We need to create new directories, and modify the code to refer to these new directories.
# We also need to modify the processing of DF_FILE "pdf_text.csv", b/c we modify DF_FILE in-place.
# We use status processed7 as the start, and we set status processed8 as the end-state.
#
#

DF_FILE=str(OUTPUTS / "pdf_text.csv")
DIR_PATH=str(PDF_TXT)
TABLE_DIR_PATH=str(PDF_TABLES_2025)
...
def one_llm_extract_table(text_content):
    llmModel = ChatOpenAI(model_name=settings.model, max_tokens=3000,api_key=st.secrets['OPENAI_API_KEY'])
    ...
    The columns should be: 
      ...
      (10) whether the fungi was mycellium, fungal mass or whole fungi
    ...
    If the paper contains figures that have additional information - note that in column 11
...
if st.button("Extract 2025 files : Stage 1"):
    ...
    for i in range(filecount):
        ...
        try:
            ...
            print(f"Error on file {i}: {e}. Continuing...")
